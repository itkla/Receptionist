...
export default function ResetPasswordPage() {
  ...
  const token = params.token as string; // Get token from URL
  ...
  const [isValidToken, setIsValidToken] = useState<boolean | null>(null); // null = checking, true/false = result

  // --- Optional: Validate token on page load --- 
  // You could add a check here to see if the token is structurally valid 
  // or even make a quick API call to check if it exists/isn't expired before showing the form.
  // For simplicity, we'll let the main API handle full validation on submit.
  useEffect(() => {
      if (!token || typeof token !== 'string' || token.length !== 64) { // Basic hex token check
          setMessage("Invalid or missing password reset token.");
          setIsValidToken(false);
      } else {
          setIsValidToken(true); // Assume valid for now, API does full check
      }
  }, [token]);
  // ----------------------------------------------
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!isValidToken) return; // Don't submit if token is known invalid
